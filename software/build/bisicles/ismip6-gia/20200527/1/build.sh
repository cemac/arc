...
#
#   svn co https://anag-repo.lbl.gov/svn/BISICLES/public/branches/ISMIP6-AIS
...
# verion information:
#
#  bisicles/ismip6 20200527: 
#
#    > r3933 | dmartin | 2020-05-27 08:09:40 +0100 (Wed, 27 May 2020) | 3 lines
#    >
#    > oops. Probably should have spellchecked this earlier...
#
...
# this version of the code has been patched from the BISICLES ISMIP6 branch
#
# the gia patches from the GIANT-BISICLES branch were then applied.
# code is on GitHub at:
#
#    https://github.com/cemacrr/bisicles_ismip6_gia/
...
# app information:
APP_NAME='bisicles/ismip6-gia'
APP_VERSION='20200527'
# build version:
BUILD_VERSION='1'
# top level build dir:
TOP_BUILD_DIR=$(pwd)
...
# get sources:
get_file 'https://ftp.mcs.anl.gov/pub/petsc/release-snapshots/petsc-lite-3.13.6.tar.gz'
...
function build_bisicles() {
  ...
  if [ ! -e ${BISICLES_HOME}/BISICLES ] ; then
    ...
    # extract!:
    tar xzf ${SRC_DIR}/bisicles-20200527.tar.gz \
      -C ${BISICLES_HOME}
    \mv ${BISICLES_HOME}/bisicles-20200527 \
...
do
  ...
  do
    ...
    if [ ! -e ${INSTALL_DIR}/petsc/lib/libpetsc.so ] ; then
      ...
      # set up build dir:
      cd ${BUILD_DIR}
      rm -fr ${BUILD_DIR}/petsc-3.13.6
      # extract source:
      tar xzf ${SRC_DIR}/petsc-lite-3.13.6.tar.gz
      cd petsc-3.13.6
      # configure and build:
      ./configure \
        ...
        --with-ssl=0  && \
        make -j8 \
        PETSC_DIR=${BUILD_DIR}/petsc-3.13.6 \
