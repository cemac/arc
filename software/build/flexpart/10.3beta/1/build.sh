...
get_file 'https://www.flexpart.eu/downloads/63' flexpart_v10.3beta.tar.gz
...
function build_flexpart() {
  ...
  # set up build dir:
  cd ${BUILD_DIR}
  rm -fr ${BUILD_DIR}/flexpart_10.3
  tar xzf ${SRC_DIR}/flexpart_v10.3beta.tar.gz
...
function build_flex_extract() {
  ...
  # set up build dir:
  cd ${BUILD_DIR}
  if [ ! -d flexpart_10.3 ] ; then
    rm -fr ${BUILD_DIR}/flexpart_10.3
    tar xzf ${SRC_DIR}/flexpart_v10.3beta.tar.gz
    \mv flexpart_v10* flexpart_10.3
  fi
  cd flexpart_10.3/preprocess/flex_extract/src
  # build and install:
  if [ "${MY_CMP}" = "intel" ] ; then
