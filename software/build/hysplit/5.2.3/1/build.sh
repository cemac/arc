...
if [ ! -e ${INSTALL_DIR}/bin/hycs_std ] ; then
  ...
  # set up build dir:
  cd ${BUILD_DIR}
  rm -fr ${BUILD_DIR}/hysplit.v5.2.3_CentOS7.9.2009/
  # extract source:
  tar xzf ${SRC_DIR}/hysplit.v5.2.3_CentOS7.9.2009.tar.gz
  # move files in to place:
  \mv hysplit.v5.2.3_CentOS7.9.2009/* ${INSTALL_DIR}/
...
if [ ! -e ${INSTALL_DIR}/bin/__py_wrapper ] ; then
  ...
  mamba update -c conda-forge -c defaults -y --all
  mamba create -n hysplit -c conda-forge -c defaults -y \
    cartopy matplotlib owslib pillow pyqt scipy numpy geopandas descartes \
    pytz 'timezonefinder==4.4.1' 'contextily==1.0.1' mercantile
  ...
  sed -i 's|^\(import path_modification\)|from timezonefinder \1|g' \
    ${CONDA_PREFIX}/lib/python*/site-packages/timezonefinder/file_converter.py
  python -m timezonefinder.file_converter
  \cp *.bin \
    ${CONDA_PREFIX}/lib/python*/site-packages/timezonefinder/
